<template>
  <div class="widget">
    ...
    <div class="widget__items">
      <div
        class="widget__item"
        v-for="(item, i) in dataTopExpensivePay"
        :key="i"
      >
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const isPreload = computed<boolean>(() => {
      return store.state.loadTopExpensivePay;
    });

    const dataTopExpensivePay = computed<boolean>(() => {
      return store.state.listTopExpensivePay;
    });
    ...
    onMounted(() => {
      store.dispatch("getTopExpensivePay").catch((err) => {
        showAlert(
          "Топ дорогих покупок",
